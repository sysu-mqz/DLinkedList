...
public:
	...
	void insertAsFirst(T data_);//前向，后向插入
	void insertAsLast(T data_);
	void sort();//按data排序
	void reverse();//反向
...
template<class T>
DLinkedList<T>::DLinkedList(DLinkedList<T>& d)//拷贝函数
{
	head = d.head;
	count = d.count;
}
...
template<class T>
void DLinkedList<T>::insertAsFirst(T Data_)//插入前头
{
	if (!empty())
	{
...
template<class T>
void DLinkedList<T>::insertAsLast(T Data_)//1插入末尾
{
	if (empty())
	{
		insertAsFirst(Data_);
